...
async def run_test_suite(llm: AzureChatOpenAI, configuration: TestConfigurationSettings, test_suite: TestSuite) -> list[TestResult]:
    suite_results = list()
...
async def run_test_series(llm: AzureChatOpenAI, configuration: TestConfigurationSettings, suite_name: str, test_series: TestSeries) -> list[TestResult]:
    series_results = list()
    browser = Browser()
    ...
    async with await browser.new_context(
        ...
        for test in test_series.tests:
            print(context.config.allowed_domains)
...
async def run_test(llm: AzureChatOpenAI, context: BrowserContext, configuration: TestConfigurationSettings, suite_name: str, series_name: str, test: Test) -> TestResult:
    try:
        task = TEST_PROMPT.format(act=test.act, expect=test.expect)
        initial_actions = [{
            "open_tab": { "url": configuration.allowed_domains[0]}
        }]
